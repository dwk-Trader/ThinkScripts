...
def primaryBuy = if FastSMA crosses above SlowSMA then yes else no;
plot ArrowUp =  FastSMA crosses above SlowSMA;
...
def cA2Sell = if (close[-1] < SlowSMA) or (close[-1] < FastSMA) or (close[-1] < SlowSMA) then yes else no;
